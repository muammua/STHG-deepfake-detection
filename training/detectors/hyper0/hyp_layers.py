...
class HyperbolicGraphConvolution(nn.Module):
    ...
    def __init__(self, manifold, in_features, in_structure, out_features, c_in, num_layers, args, is_hypergraph):
        ...
        self.agg = HypAgg(manifold, c_in, in_features, in_structure, out_features, num_layers, args,
                          is_hypergraph)  # .to(args.device)

    def forward(self, input):
        data, is_hypergraph = input
        hyperbolic_n_f = self.agg.forward(data, is_hypergraph)

        return hyperbolic_n_f
...
class HypAgg(Module):
    ...
    def __init__(self, manifold, c, in_features, in_structure, out_features, num_layers, args, is_hypergraph):
        ...
        if is_hypergraph:
            self.stackGCNs_f = getattr(
                Encoder_HyperGNN(in_features, out_features, num_layers, args.pooling, args.readout).to(device),
                'forward')
            self.stackGCNs_s = getattr(
                Encoder_HyperGNN(in_structure, out_features, num_layers, args.pooling, args.readout).to(device),
                'forward')
        else:
            if args.GNN_Encoder == 'GCN':
                self.stackGCNs_f = getattr(
                    Encoder_GCN(in_features, out_features, num_layers, args.pooling, args.readout).to(device),
                    'forward')
                self.stackGCNs_s = getattr(
                    Encoder_GCN(in_structure, out_features, num_layers, args.pooling, args.readout).to(device),
                    'forward')
            else:
                self.stackGCNs_f = getattr(
                    Encoder_GIN(in_features, out_features, num_layers, args.pooling, args.readout).to(device),
                    'forward')
                self.stackGCNs_s = getattr(
                    Encoder_GIN(in_structure, out_features, num_layers, args.pooling, args.readout).to(device),
                    'forward')
    ...
    def forward(self, data, is_hypergraph):
        ...
        if is_hypergraph:
            n_tan_f = self.stackGCNs_f(x_tangent_f, data.hyperedge_index, data.batch)
        else:
            n_tan_f = self.stackGCNs_f(x_tangent_f, data.edge_index, data.batch)
...
class Encoder_HyperGNN(torch.nn.Module):
    ...
    def forward(self, x, hyperedge_index, batch):
        ...
        if self.readout == 'last':
            ...
            return torch.sum(xs_tensor, dim=0)
